...
function TLHalfDayRequest() {
    ...
    const handleStatusButtonClick = (row, status) => {
        ...
        fetch('https://epkgroup.in/crm/api/public/api/tl_approval_halfday_request', {
            method: 'POST',
            body: formData,
            headers: {
                ...
                if (data.status === 'success') {
                    ...
                    if (status === 'Approved') {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success',
                            text: 'HalfDay Request Successfully Approved',
                        });
                    } else if (status === 'Rejected') {
                        Swal.fire({
                            icon: 'error',
                            title: 'Rejected',
                            text: 'HalfDay Request Rejected',
                        });
                    }
                    ...
                    console.error('HalfDay Request Approval failed:', data.message);

                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'HalfDay Request Approval failed',
                    });
                }
            })
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/tl_halfdayrequest_list', {
                method: 'POST',
                headers: {
    ...
    const handleExportCSV = () => {
        const csvData = tableData.map(({ emp_name, departmentName, category_name, shift_slot, permission_date, permission_timefrom, permission_timeto, leave_reason, emp_status, tl_status }, index) => ({
            '#': index + 1,
            emp_name,
            departmentName,
            category_name,
    ...
    const handleExportPDF = () => {
        ...
        const data = tableData.map(({ emp_name, departmentName, category_name, shift_slot, permission_date, permission_timefrom, permission_timeto, leave_reason, emp_status, tl_status }, index) => [
            index + 1,
            emp_name,
            departmentName,
            category_name,
        ...
        doc.autoTable({
            head: [['S.No', 'Name', 'Department', 'Category', 'Shift slot', 'Date', 'From Time', 'To Time', 'Reason', 'HR Status', 'TL Status']],
            body: data,
            // styles: { fontSize: 10 },
            // columnStyles: { 0: { halign: 'center', fillColor: [100, 100, 100] } }, 
        });

        doc.save('TLHalfdayRequest.pdf');
        ...
        <>
            ...
            {loading ? (
                ...
                <Container fluid className='shift__container'>
                    <h3 className='mb-5' style={{ fontWeight: 'bold', color: '#00275c' }}>HalfDay Request List</h3>
