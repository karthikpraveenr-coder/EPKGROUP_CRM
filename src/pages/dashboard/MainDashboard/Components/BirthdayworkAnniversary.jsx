    ...
    useEffect(() => {
        ...
        const fetchEmployeeEvents = async () => {
            try {
                ...
                const response = await axios.get('https://epkgroup.in/crm/api/public/api/adminIndexTodayCount', {
                    headers: {
                ...
                {activeData.slice(0, 3).map(item => (
                    <div className='Birthdaylist__container mb-2' key={item.id}>
                        <span className='Birthdaylist'>
                            <img src={`https://epkgroup.in/crm/api/storage/app/${item.profile_img}`} alt='userimage' className='userimage' />
