...
function AddCustomer() {
    ...
    const GoToTaskListPage = () => {
        switch (selectedModule) {
            case '1':
                navigate('/admin/trialpacklist');
                break;
            case '2':
                navigate('/admin/presalesbuypacklist');
                break;
            default:
                console.error('Unknown selectedModule value');
                break;
        }
    };
    ...
    const handleSubmit = (e) => {
        ...
        const payload = {
            ...
            total_plan_amt: totalAfterDiscount,
        ...
        axios.post('https://epkgroup.in/crm/api/public/api/office3i_add_lead', payload, {
            headers: {
    ...
    useEffect(() => {
        if (selectedCountry) {
            axios.get(`https://epkgroup.in/crm/api/public/api/office3i_state_list/${selectedCountry.value}`, {
                headers: {
                    'Authorization': `Bearer ${usertoken}`
                }
            })
    ...
    useEffect(() => {
        if (selectedState) {
            axios.get(`https://epkgroup.in/crm/api/public/api/office3i_city_list/${selectedState.value}`, {
                headers: {
                    'Authorization': `Bearer ${usertoken}`
                }
            })
    ...
    return (
        <div>
            <Form onSubmit={handleSubmit} style={{ padding: '20px 30px 0px 30px' }}>
                <h3 className='mb-4' style={{ fontWeight: 'bold', color: '#00275c' }}>Add Customer</h3>
