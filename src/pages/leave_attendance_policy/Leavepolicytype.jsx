...
function Leavepolicytype() {
    ...
    const GoToEditPage = (id) => {
        navigate(`/admin/editleavepolicytype/${id}`);
    };
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('https://epkgroup.in/crm/api/public/api/leavetypeinsert', requestData, {
            headers: {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/view_leavetype', {
                headers: {
                    'Authorization': `Bearer ${usertoken}`
                }
            });
    ...
    const handleDelete = async (id) => {
        try {
            const { value: reason } = await Swal.fire({
                title: 'Are you sure?',
                text: 'You are about to delete this Leave Policy Type. This action cannot be reversed.',
                icon: 'warning',
                input: 'text',
            ...
            if (reason) {
                const response = await fetch('https://epkgroup.in/crm/api/public/api/delete_leavetype', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                if (response.ok || response.type === 'opaqueredirect') {
                    ...
                    Swal.fire('Deleted!', 'Leave Policy Type has been deleted.', 'success');
                } else {
                    throw new Error('Error deleting Leave Policy Type');
                }
            }
        } catch (error) {
            console.error('Error deleting Leave Policy Type:', error);
            Swal.fire('Error', 'An error occurred while deleting the leave type. Please try again later.', 'error');
        }
    };
        ...
        <>
            ...
            {loading ? (
                ...
                <Container fluid className='shift__container'>
                    ...
                    {/* ------------------------------------------------------------------------------------------------ */}
                    {/* List table */}
                    <h3 className='mb-5'> Leave Policy Type List</h3>
...
export default Leavepolicytype