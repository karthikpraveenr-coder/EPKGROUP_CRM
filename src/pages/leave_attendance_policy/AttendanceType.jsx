...
function AttendanceType() {
    ...
    const GoToEditPage = (id) => {
        navigate(`/admin/editattendancetype/${id}`);
    };
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('https://epkgroup.in/crm/api/public/api/attendancetypeinsert', requestData, {
            headers: {
            ...
            .catch(error => {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'There was an error creating the Attendance request type. Please try again later.',
                });
    ...
    const fetchData = async () => {
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/view_attendancetype', {
                headers: {
                    'Authorization': `Bearer ${usertoken}`
                }
            });
    ...
    const handleDelete = async (id) => {
        try {
            const { value: reason } = await Swal.fire({
                title: 'Are you sure?',
                text: 'You are about to delete this Attendance Type. This action cannot be reversed.',
                icon: 'warning',
                input: 'text',
            ...
            if (reason) {
                const response = await fetch('https://epkgroup.in/crm/api/public/api/delete_attendancetype', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                if (response.ok || response.type === 'opaqueredirect') {
                    ...
                    Swal.fire('Deleted!', 'Attendance Type has been deleted.', 'success');
                } else {
                    throw new Error('Error deleting Attendance Type');
                }
            }
        } catch (error) {
            console.error('Error deleting Attendance Type:', error);
            Swal.fire('Error', 'An error occurred while deleting the Attendance Type. Please try again later.', 'error');
        }
    };
    ...
    const handleExportCSV = () => {
        ...
        const headers = [
            { label: 'S.No', key: '#' },
            { label: 'Request Type Name', key: 'request_type_name' },
    ...
    const handleExportPDF = () => {
        ...
        doc.autoTable({
            head: [['S.No', 'Request type name', 'Status', 'Created By', 'Updated By']],
            body: data,
            // styles: { fontSize: 10 },
            // columnStyles: { 0: { halign: 'center', fillColor: [100, 100, 100] } }, 
        });

        doc.save('Attendancetype.pdf');
        ...
        <>
            ...
            {loading ? (
                ...
                <Container fluid className='shift__container'>
                    <h3 className='mb-5'>Add Attendance Type</h3>
                        ...
                        <Col>
                            <Form>
                                <Form.Group controlId="formShift">
                                    ...
                                    <Form.Control type="text" placeholder="Enter Attendance Type" value={attendancetype} onChange={(e) => handleInputChange(setAttendancetype)(e)} />
                    ...
                    <h3 className='mb-5'>Attendance Type List</h3>
