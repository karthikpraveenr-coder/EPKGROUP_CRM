...
function AddShiftSlot() {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('https://epkgroup.in/crm/api/public/api/shiftslotinsert', requestData, {
            headers: {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/view_shiftslot', {
                headers: {
                    'Authorization': `Bearer ${usertoken}`
                }
            });
    ...
    const handleDelete = async (id) => {
        try {
            const { value: reason } = await Swal.fire({
                title: 'Are you sure?',
                text: 'You are about to delete this shift slot. This action cannot be reversed.',
                icon: 'warning',
                input: 'text',
            ...
            if (reason) {
                const response = await fetch('https://epkgroup.in/crm/api/public/api/delete_shiftslot', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                if (response.ok || response.type === 'opaqueredirect') {
                    ...
                    Swal.fire('Deleted!', 'Shift slot has been deleted.', 'success');
                } else {
            ...
            Swal.fire('Error', 'An error occurred while deleting the shift slot. Please try again later.', 'error');
        }
    };
    ...
    const handleExportCSV = () => {
        const csvData = tableData.map(({ shift_slot, status, created_name, updated_name }, index) => ({
            '#': index + 1,
            shift_slot,
            status,
            created_name,
    ...
    const handleExportPDF = () => {
        ...
        const data = tableData.map(({ shift_slot, status, created_name, updated_name }, index) => [
            index + 1,
            shift_slot,
            status,
            created_name,
        ...
        doc.autoTable({
            head: [['S.No', 'Shift Slot', 'Status', 'Created By', 'Updated By']],
            body: data,
            // styles: { fontSize: 10 },
        ...
        <>
            ...
            {loading ? (
                ...
                <Container fluid className='shift__container'>
                    <h3 className='mb-5'>Add Shift Slot</h3>
                    ...
                    {/* ------------------------------------------------------------------------------------------------ */}
                    {/* List table */}
                    <h3 className='mb-5'>Shift Slot List</h3>
