...
function OvertimeType() {
    ...
    const GoToEditPage = (id) => {
        navigate(`/admin/editovertimeType/${id}`);
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (!ottype) {
            errors.ottype = 'Ot Type is required.';
        }
        ...
        axios.post('https://epkgroup.in/crm/api/public/api/add_ot_requesttype', requestData, {
            headers: {
            ...
            .catch(error => {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'There was an error creating the Overtime type. Please try again later.',
                });
    ...
    const fetchData = async () => {
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/view_ot_requesttype', {
                headers: {
                    'Authorization': `Bearer ${usertoken}`
                }
            });
    ...
    const handleDelete = async (id) => {
        try {
            const { value: reason } = await Swal.fire({
                title: 'Are you sure?',
                text: 'You are about to delete this Overtime Type. This action cannot be reversed.',
                icon: 'warning',
                input: 'text',
            ...
            if (reason) {
                const response = await fetch('https://epkgroup.in/crm/api/public/api/delete_ot_requesttype', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                if (response.ok || response.type === 'opaqueredirect') {
                    ...
                    Swal.fire('Deleted!', 'Overtime Type has been deleted.', 'success');
                } else {
                    throw new Error('Error deleting Overtime Type');
                }
            }
        } catch (error) {
            console.error('Error deleting Overtime Type:', error);
            Swal.fire('Error', 'An error occurred while deleting the Overtime Type. Please try again later.', 'error');
        }
    };
    ...
    const handleExportCSV = () => {
        const csvData = tableData.map(({ request_type_name, status, created_name, updated_name }, index) => ({
            '#': index + 1,
            request_type_name,
            status,
            created_name,
        ...
        const headers = [
            { label: 'S.No', key: '#' },
            { label: 'OT Type', key: 'request_type_name' },
            { label: 'Status', key: 'status' },
            { label: 'OT Type', key: 'created_name' },
            { label: 'OT Type', key: 'updated_name' },
        ];

        const csvReport = {
            data: csvData,
            headers: headers,
            filename: 'Overtimetype.csv',
        };
    ...
    const handleExportPDF = () => {
        ...
        const data = tableData.map(({ request_type_name, status, created_name, updated_name }, index) => [
            index + 1,
            request_type_name,
            status,
            created_name,
        ...
        doc.autoTable({
            head: [['S.No', 'OT Type', 'Status', 'Created By', 'Updated By']],
            body: data,
            // styles: { fontSize: 10 },
            // columnStyles: { 0: { halign: 'center', fillColor: [100, 100, 100] } }, 
        });

        doc.save('Overtimetype.pdf');
        ...
        <>
            ...
            {loading ? (
                ...
                <Container fluid className='shift__container'>
                        ...
                        <Col>
                            <Form>
                                <Form.Group controlId="formShift">
                                    <Form.Label style={{ fontWeight: 'bold' }}>OT Type <sup><FontAwesomeIcon icon={faStarOfLife} style={{ color: '#fb1816', fontSize: '8px' }} /></sup></Form.Label>
                                    <Form.Control type="text" placeholder="Enter OT Type" value={ottype} onChange={(e) => handleInputChange(setOTtype)(e)} />
                    ...
                    {/* ------------------------------------------------------------------------------------------------ */}
                    {/* List table */}
                    <h3 className='mb-5'>Overtime Type List</h3>
