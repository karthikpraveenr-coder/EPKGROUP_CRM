...
export default function AddPurchaseInvoice() {
    ...
    const handleSubmit = async (e) => {
        ...
        formDataToSend.append('vendor_id', vendorname?.value);
        formDataToSend.append('ship_to', shipto?.value);
        formDataToSend.append('bill_to', billto?.value);
        ...
        // formDataToSend.append('delivery_date', deliveryDate || '');
        formDataToSend.append('delivery_date', deliveryDate || '');


        formDataToSend.append('reference_no_date', referenceNoAndDates  || '-');
        formDataToSend.append('other_reference', otherReference || '-');
        formDataToSend.append('buyers_order_no', buyersOrderNo || 0);
        formDataToSend.append('order_date', buyersOrderDate  || '');
        formDataToSend.append('dispatch_doc_no', dispatchDocNo || 0);

        formDataToSend.append('mode_termsof_payment', modeOfPayment || 0);
    ...
    useEffect(() => {
        ...
        fetch('https://epkgroup.in/crm/api/public/api/sales_company_list', {
            headers: {
                'Authorization': `Bearer ${usertoken}`
            }
        })
    ...
    //     fetch('https://epkgroup.in/crm/api/public/api/autogeneratesaleinvoiceid', {
    //         headers: {
    //             'Authorization': `Bearer ${usertoken}`
    //         }
    //     })
    ...
    useEffect(() => {
        fetch('https://epkgroup.in/crm/api/public/api/sales_item_list', {
            headers: {
                'Authorization': `Bearer ${usertoken}`
            }
        })
    ...
    //         fetch(`https://epkgroup.in/crm/api/public/api/sales_hsn_sac/${descriptionalGoods.value}`, {
    //             headers: {
    //                 'Authorization': `Bearer ${usertoken}`
    //             }
    //         })
    ...
    useEffect(() => {
        items.forEach((item, index) => {
            if (item.descriptionalGoods) {
                fetch(`https://epkgroup.in/crm/api/public/api/sales_hsn_sac/${item.descriptionalGoods.value}`, {
                    headers: {
                        'Authorization': `Bearer ${usertoken}`
                    }
                })
    ...
    return (
        <div className="container mt-5" style={{ padding: '0px 70px 0px' }}>
            <h3 className='mb-5' style={{ fontWeight: 'bold', color: '#00275c' }}>Add Purchase Invoice</h3>
            <div style={{ boxShadow: '#0000007d 0px 0px 10px 1px', padding: '35px 50px' }} className='mb-5'>
                <form onSubmit={handleSubmit}>
                    ...
                    <button type="submit" className="btn btn-primary" style={{ marginRight: '10px' }}>Add Purchase Invoice</button>
