...
export default function SalesInvoice() {
    ...
    useEffect(() => {
        ...
        axios.get(`https://epkgroup.in/crm/api/public/api/editview_saleinvoice/${id}`, {
            headers: {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/update_saleinvoice', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${usertoken}`
    ...
    useEffect(() => {
        const fetchHsnSacOptions = async () => {
            ...
            for (const [index, item] of items.entries()) {
                if (item.descriptionalGoods) {
                    try {
                        const response = await fetch(`https://epkgroup.in/crm/api/public/api/sales_hsn_sac/${item.descriptionalGoods}`, {
                            headers: { 'Authorization': `Bearer ${usertoken}` }
                        });
                        const data = await response.json();
                        if (data.status === 'success') {
    ...
    const handleChange = async (index, field, value) => {
        ...
        if (field === 'descriptionalGoods') {
            ...
            if (selectedOption) {
                try {
                    const response = await fetch(`https://epkgroup.in/crm/api/public/api/sales_hsn_sac/${value}`, {
                        headers: { 'Authorization': `Bearer ${usertoken}` }
                    });
                    const data = await response.json();
                    if (data.status === 'success') {
