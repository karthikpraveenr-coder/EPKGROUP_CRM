...
function BuyPackList() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/getonline_buybowpacklist', {
                method: 'GET',
                headers: {
    ...
    const handleExportCSV = () => {
        const csvData = tableData.map(({ cus_name, cus_email, plan_name, add_emp_count,total_plan_amt, payment_methodname, payment_statusname, start_date, end_date }, index) => ({
            '#': index + 1,
            cus_name,
            cus_email, 
            plan_name, 
        ...
        const headers = [
            ...
            { label: 'Plan Amount', key: 'total_plan_amt' },
    ...
    const handleExportPDF = () => {
        ...
        const data = tableData.map(({ cus_name, cus_email, plan_name, add_emp_count,total_plan_amt, payment_methodname, payment_statusname, start_date, end_date }, index) => [
            index + 1,
            cus_name,
            cus_email, 
            plan_name, 
        ...
        doc.autoTable({
            head: [['S.No', 'Name', 'Email', 'Plan Name', 'AddOn Employee Count', 'Plan Amount', 'Payment Method', 'Payment Status', 'Start Date', 'End Date']],
            body: data,
