...
function PresalesBuyPackList() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/getbuynow_packlist', {
                method: 'POST',
                headers: {
    ...
    const handlePayButtonClick = (row) => {
        ...
        axios.post('https://epkgroup.in/crm/api/public/api/sent_paylink', payload, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${usertoken}`,
            }
