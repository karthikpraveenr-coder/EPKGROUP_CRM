...
function AddDocumentType() {
    ...
    const GoToEditPage = (id) => {
        navigate(`/admin/editdocumenttype/${id}`);
    };
    ...
    // ------------------------------------------------------------------------------------------------
    // Add Employee Document Type submit
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('https://epkgroup.in/crm/api/public/api/empdoctypeinsert', requestData, {
            headers: {
            ...
            .catch(error => {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'There was an error creating the Employee Document Type. Please try again later.',
                });
    ...
    const fetchData = async () => {
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/view_empdoctype', {
                headers: {
                    'Authorization': `Bearer ${usertoken}`
                }
            });
    ...
    const handleDelete = async (id) => {
        try {
            const { value: reason } = await Swal.fire({
                title: 'Are you sure?',
                text: 'You are about to delete this Employee Document Type. This action cannot be reversed.',
                icon: 'warning',
                input: 'text',
            ...
            if (reason) {
                const response = await fetch('https://epkgroup.in/crm/api/public/api/delete_empdoctype', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                if (response.ok || response.type === 'opaqueredirect') {
                    ...
                    Swal.fire('Deleted!', 'Employee Document Type has been deleted.', 'success');
                } else {
                    throw new Error('Error deleting Employee Document Type');
                }
            }
        } catch (error) {
            console.error('Error deleting Employee Document Type:', error);
            Swal.fire('Error', 'An error occurred while deleting the Employee Document Type. Please try again later.', 'error');
        }
    };
    ...
    const handleExportCSV = () => {
        const csvData = tableData.map(({ document_name, status, created_name, updated_name }, index) => ({
            '#': index + 1,
            document_name,
            status,
            created_name,
        ...
        const headers = [
            { label: 'S.No', key: '#' },
            { label: 'Emp Document Name', key: 'document_name' },
        ...
        const csvReport = {
            data: csvData,
            headers: headers,
            filename: 'DocumenttypeReport.csv',
        };
    ...
    const handleExportPDF = () => {
        ...
        const data = tableData.map(({ document_name, status, created_name, updated_name }, index) => [
            index + 1,
            document_name,
            status,
            created_name,
        ...
        doc.autoTable({
            head: [['S.No', 'Emp Document Name', 'Status', 'Created By', 'Updated By']],
            body: data,
            // styles: { fontSize: 10 },
            // columnStyles: { 0: { halign: 'center', fillColor: [100, 100, 100] } }, 
        });

        doc.save('EmpdocumenttypeReport.pdf');
        ...
        `}
            ...
            {loading ? (
                ...
                <Container fluid className='shift__container'>
                    <h3 className='mb-5' style={{ fontWeight: 'bold', color: '#00275c' }}>Employee Document Type</h3>

                    {/* ------------------------------------------------------------------------------------------------ */}
                    {/* Emp Document Type add form */}
                    <h5 className='mb-3' style={{ fontWeight: 'bold', color: '#00275c' }}>Add Employee Document Type </h5>
                    <Row className='mb-5 shift__row'>
                        <Col>
                            <Form>
                                <Form.Group controlId="formShift">
                                    <Form.Label style={{ fontWeight: 'bold' }}>Document Name <sup><FontAwesomeIcon icon={faStarOfLife} style={{ color: '#fb1816', fontSize: '8px' }} /></sup></Form.Label>
                                    <Form.Control type="text" placeholder="Enter Document Name (Ex: PAN)" value={empdoctype} onChange={(e) => handleInputChange(setEmpdoctype)(e)} />
                                </Form.Group>
                        ...
                        <table className="table">
                            <thead className="thead-dark">
                                <tr>
                                    ...
                                    <th scope="col">Employee Document Name</th>
