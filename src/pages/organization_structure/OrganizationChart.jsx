...
export default function ColoredDemo() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('https://epkgroup.in/crm/api/public/api/orgchart_emp_list/0', {
                    headers: {
    ...
    const formatData = (data) => {
        return data.map(item => ({
            ...
            data: {
                image: item.profile_img ? `https://epkgroup.in/crm/api/storage/app/${item.profile_img}` : 'https://via.placeholder.com/150',
            ...
            children: item.subordinates ? item.subordinates.map(sub => ({
                ...
                style: { borderRadius: '12px' },
                data: {
                    image: sub.profile_img ? `https://epkgroup.in/crm/api/storage/app/${sub.profile_img}` : 'https://via.placeholder.com/150',
    ...
    const datapop = async (id) => {
        ...
        try {
            const response = await axios.get(`https://epkgroup.in/crm/api/public/api/orgchart_employee_data/${id}`, {
                headers: {
                    'Authorization': `Bearer ${usertoken}`
                }
            });
