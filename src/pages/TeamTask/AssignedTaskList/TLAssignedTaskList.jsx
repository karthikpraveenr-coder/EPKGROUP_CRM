...
function TLAssignedTaskList() {
    ...
    const GoToEditPage = (id) => {
        navigate(`/admin/edittlassignedtasklist/${id}`);
    };
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/view_assigntask_list', {
    ...
    useEffect(() => {
        const fetchrole = async () => {
            try {
                const response = await axios.get(`https://epkgroup.in/crm/api/public/api/supervisor_department_list/${userempid}`, {
                    headers: {
                        'Authorization': `Bearer ${usertoken}`
                    }
                });
        ...
        <>
            ...
            {loading ? (
                ...
                <Container fluid className='shift__container'>
                    <h3 className='mb-5' style={{ fontWeight: 'bold', color: '#00275c' }}>TL Assigned Task List</h3>
...
export default TLAssignedTaskList