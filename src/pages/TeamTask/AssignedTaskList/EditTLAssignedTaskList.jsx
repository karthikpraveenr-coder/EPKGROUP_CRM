...
const EditTLAssignedTaskList = () => {
    ...
    const handleVisittasklist = () => {
        navigate(`/admin/tlassignedtasklist`);
    };
    // loading state
    ...
    useEffect(() => {
        if (attachment) {
            document.querySelector("#img").setAttribute("src", `https://epkgroup.in/crm/api/storage/app/${attachment}`);
        }
    }, [attachment]);
    ...
    const handleSave = (e) => {
        ...
        axios.post('https://epkgroup.in/crm/api/public/api/update_task_list', formData, {
            headers: {
            ...
            .catch(error => {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'There was an error creating the Assign Task. Please try again later.',
                });
    ...
    useEffect(() => {
        axios.get(`https://epkgroup.in/crm/api/public/api/edit_tasklist/${id}`, {
            headers: {
                'Authorization': `Bearer ${usertoken}`
            }
        })
    ...
    useEffect(() => {
        const fetchProjectNames = async () => {
            try {
                const response = await axios.get('https://epkgroup.in/crm/api/public/api/project_name_list', {
                    headers: {
                        'Authorization': `Bearer ${usertoken}`
                    }
                });
    ...
    return (
        <div className='Addproject__container mt-5' style={{ padding: '0px 70px 30px' }}>
            <h3 className='mb-5' style={{ fontWeight: 'bold', color: '#00275c' }}>Edit Assigned Task</h3>
...
export default EditTLAssignedTaskList;
