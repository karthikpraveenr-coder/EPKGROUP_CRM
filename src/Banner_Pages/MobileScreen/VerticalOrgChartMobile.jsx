...
import './VerticalOrgchartMobile.css';
    ...
    const fetchHierarchy = async () => {
      try {
        const response = await axios.get(
          "http://epkgroup.in/crm/api/public/api/employee_hierarchy_mobile");
  ...
  const handleNodeDetails = async (employee) => {
    ...
    if (employee.id.length >= 2) {
      try {
        ...
        const response = await axios.get(
          `http://epkgroup.in/crm/api/public/api/orgchart_employee_data_mobile/${employee.emp_id}`);
        ...
        <Modal show={showModal} onHide={() => setShowModal(false)}>
          <Modal.Header closeButton>
            <div className="image__name">
              <img
                src={`https://epkgroup.in/crm/api/storage/app/${employee_details?.profile_img}`}
...
export default VerticalOrgChartMobile;
